...
function repearArr(num) {
    ...
    num.map(date => {
        if(date.length != 10) { // проверка на дленну элемента
            return false
        } else if(date.length == 10) { 
            //console.log(date[2],date[5])
            if(date[2] == '/' || date[2] == '-' && date[5] == '/' || date[5] == '-') { // проверка на нахождение нужных соединяющих даты элементов '/' и '-'
                if(date[0]+date[1] != '00' && date[3]+date[4] != '00' && date[0]+date[1] <= 31 && date[3]+date[4] <= 12) { 
                    // проверка, что даты не начинаются на 00 в днях и 00 месяцах
                    // также проверка, что дни и месяца не больше 31 и 12
                    if(date[0]+date[1] >= 28 && date[3]+date[4] == 2 ) { // отдельная проверка для Февраля 
                        return false
                    } else {
                        date = date.split('/').join('-'); // заменяем через метод split() '/'и добавляем - 
                        positiveArr.push(date) // добавляем в новых массив все элементы прошедшие проверку
                    }
                }
            }
        } else {
...
const arr = ['10-02-2022', 'тест', '11/12/2023', '00/13/2022', '41/12/2023', '11/00/2023', '110/0/2023', 'wertyuiopa', '29-12-1987', '30/11/1990', '27/12-1984', '12-01/1964', '30-02-1988', '27-02-1955', 'S7-g2-1955'];
